...
def augment_json_with_paddle(
    ...
    try:
        ...
        # --- load JSON ---
        ocr_json_path_ = Path(ocr_json_path)
        ...
        for entry in ocr_data:
            rel_path = Path(entry["image_rel_path_from_root"])
            fname = entry["image_file_name"]
            img_path = img_root_folder / rel_path / fname

            if not img_path.exists():
                entry["paddleocr_result"] = {"error": f"Image not found at {img_path}"}
                continue
            ...
            if use_process_and_save:
                ...
                entry["paddleocr_result"] = results
            else:
                # Minimal run
                result = ocr.predict(input=str(img_path))
